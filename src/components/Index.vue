<template>
    <div class="index-vue">
        ...
        <aside :class="asideClassName">
            <!-- logo -->
            <div class="logo-c">
                <span v-show="isShowAsideTitle">汽车美护系统</span>
            </div>
            <!-- 菜单栏 -->
    ...
    methods: {
        ...
        changePwds(){
            ...
            modalSure(pwdForm){
                ...
                if(pwdForm.newPassword1!==pwdForm.newPassword){
                    this.$message({
                        message:'新密码不一致！',
                        type:'error'
                    })
                }else{
                    this.$confirm('修改后将启用新密码，是否修改?', '提示', {
                        ...
                        changePwd({password:pwdForm.newPassword,id}).then(resp=>{
                            if(resp.code==200){
                                this.isShowpwdModal=false
                            }
                            this.$message({
