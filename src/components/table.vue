<template>
    <div class="tabel-container">
        ...
        >
            <template>
                <el-table-column
                    ...
                    <template slot-scope="scope">
                        ...
                        <div v-if="item.type === 'specialNumber'">
                            {{scope.row[item.attribute]+item.Symbol}}
                        </div>
                        <div v-if="item.type === 'tableTextTips'||'doubleDate'">
                            <span v-if="item.type === 'tableTextTips'">
                                {{scope.row[item.attribute]==true?'每隔'+scope.row[item.relationKeyOne]+'天最多参与'+scope.row[item.relationKeyTwo]+'次':'不限制参与次数'}}
                            </span>
                            <span v-if="item.type === 'doubleDate'">
                                {{scope.row[item.relationKeyOne]+'     至     '+scope.row[item.relationKeyTwo]}}
                            </span>
                        </div>
                        ...
                        <div v-if="item.type==='checkbox'">
                            <span>
                               <el-checkbox v-if="item.type === 'checkbox'" placeholder=""
                                            v-model="scope.row[item.attribute]"
                                            :disabled="true"
                                            :key="scope.row[item.key]"></el-checkbox>
                            </span>
                        </div>
            ...
            <el-table-column :min-width="oprWidth" v-if="needOperationArea" label="操作" class="operationArea">
                <template scope="scope">
                    ...
                    <el-button v-if="needDetail" @click="handleDetailClick(scope.row,scope.$index)" type="success"
                               code="M1005"
                               size="small" plain>{{tableDetailText}}
                    </el-button>
                    ...
                    <el-button
                        ...
                        <span v-else-if="item.itemType === 'putOnTheShelf'">
                            {{scope.row.status == '1'? '下架' : '上架'}}
                        </span>
                        <span v-else-if="item.itemType === 'setInto'">
                            {{scope.row.status == '1'? '已入库' : '未入库'}}
                        </span>
